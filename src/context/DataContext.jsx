...
export const DataProvider = ({ children }) => {
  ...
  // this function is just for to use setUsers example
  const updateGeneratorStatus = (id, status) => {
    setUsers((prev) =>
  ...
  const systemEfficiency =
    totalOutput > 0 ? (totalOutput / totalCapacity) * 100 : 0;
        ...
        prev.map((gen) => {
          if (gen.status === "online") {
            ...
            const newOutput = Math.max(
              0,
              Math.min(gen.capacity, gen.output + variation)
            );
            const tempVariation = (Math.random() - 0.5) * 5;
            const newTemp = Math.max(
              70,
              Math.min(100, gen.temperature + tempVariation)
            );
            ...
            return {
              ...
              efficiency:
                Math.round((newOutput / gen.capacity) * 100 * 10) / 10,
            };
          }
          return gen;
        })
